...
def game_label(con: duckdb.DuckDBPyConnection, athlete_id: str, event_id: str) -> str | None:
    """One player's game, described as ``"<date> vs <OPP>, <W/L> <score>-<score>"``.
    ...
    if row is None or any(value is None for value in row):
        return None
    date, opponent_abbr, won, team_score, opponent_score = row
    return f"{eastern_date(date)} vs {opponent_abbr}, {'W' if won else 'L'} {team_score}-{opponent_score}"
