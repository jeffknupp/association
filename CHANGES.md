...
## Unreleased
- **A companion the router named nobody for is read from the question's own words** (plan item 3, F087's last half). "show me splits for the sixers when maxey scores 20+ points" arrived with an invented Joel Embiid and no Maxey in any slot, and a companion was only ever read from the router's names - so the reading refused by Embiid's name where the question had an answer. `subject._unrouted_companions` takes a companion phrase's leading word (up to two) as a name where it is a whole word of some player's name and not an ordinary word, a team's or a stat's - the question's spelling, never a resolution, so the template resolves "maxey" the way it resolves any open name and says how; and the settled `record_when` counts a reached companion as the spare name that replaces the invention, as the router's own `record_when` did. Measured on the 277 recorded day9 routes: one answer moves, F087's, to the 76ers' 35-28 (the key); `port_check` unchanged (0 false positives over 263 rows of other intents).
- **`with_without` splits by any role the question gives its teammates** (plan item 3, step C, second half): "celtics record when tatum starts" is the games he started against the rest, "when embiid, maxey and edgecombe start" all three, a line ("when maxey had 20+ points") the games he reached it - each teammate counted by his own predicate (`conditions._with_without_games`), the rows named in the question's words ("Jayson Tatum started" / "did not start"), read off the `conditions` slot the reading writes (`HONORED_SCOPING["with_without"]`). A plain "when X plays" is the played/out split it always was. Golden unchanged.
- **A team's record in the games a player named beside it reached a line, and a pair's record across their careers** (plan item 3, step C, first half). The reading settles `record_when` for a team subject with a `reached` companion under any of the intents the router files it as, with the companion as the template's player ("show me splits for the sixers when maxey scores 20+ points", yardstick-v2 F087: 35-28 in his 20-point games, where the player's own splits or a refusal over an invented Joel Embiid answered); on `record_when` a reached companion is the spare name that replaces a router invention. A `player_matchup` "record" with no season named spans their careers, and a matchup an absence leaves empty says what it counted - "they met 27 times in all, 4 of them with Kevin Durant playing beside Stephen Curry; 'without Kevin Durant' counts only the games he missed while on Stephen Curry's team, and there were none among their meetings" (F114 "steph curry record vs lebron regular season without kd", which read the model's default season and answered "no meetings" with nothing more). A team named after "for" with no "vs" anywhere is the subject's own side, never the router's `opponent` ("show me splits for the sixers when maxey scores 20+ points" arrived with the 76ers as the opponent).
- **The reading gives each companion his role** (plan item 3, step B). `Subject.conditions` holds `Companion(name, predicate, stat, threshold)` for every player named beside the subject - `absent` ("without X", "with X out"), `played` ("when X and Y play"), `started`, `bench`, `reached` ("when Maxey scores 20+ points") - read off the phrase's own words; `companions` stays their names. `apply_subject` writes the roles the router's own slots cannot carry (a start, the bench, a line) as the `conditions` slot where the template honors it, never on the read's own subject (record_when groups its player's games by that line itself); an absence and a played companion stay in `without`/`with_player`, which the comparison templates read as the split's two sides. Golden unchanged.
- **The player condition on the relation** (ROADMAP plan item 3, step A). `player_games.Condition(player, side, predicate, line, tenure)` - a teammate or an opponent-side player who `played`, was `absent`, `started`, came off the `bench` or `reached` a line in the game - is one shape on `Narrowed` (`conditions`, `add_condition`, `condition_clause`); the teammate absence every relation template already read (`without`, with its tenure clause) is now the own-side absent condition, word for word the same SQL, and `without`/`tenure` are views over it. A `conditions` slot (a list of `{player, side, predicate, stat?, threshold?}`) is read by the shared step (`_condition_from_slot`, `RELATION_SCOPING`), so every relation template honors it and every other refuses it by `check_scope`. Golden over the 308 recorded questions unchanged; nothing writes the slot yet (step B, the reading).
- **The season line is the compiler's second source** (ROADMAP plan item 2, step 2a, ISSUES #228). A compiled point now carries a `source`: `"games"` (the player-games relation, as before) or `"seasons"` (`player_season_stats_deduped`, one row per player per season), chosen for an unnarrowed `player_stat` line and for a per-season `player_history`. A season-line point is said only through the templates' own readers, split out of `templates.players` for it - `_player_stat_season_line_subject`/`_player_stat_season_line` and `_player_history_subject`/`_player_history_read`, which the templates now call too - and never compiled over box scores (`compile_query` refuses it); a history the season line cannot say (a stat with no per-season column, a measure the words moved) falls back to the game-level reading as before (`compose.move.games_reading`). Parity over the recorded corpus (`parity.py`, text and `data` identical to the template on the same slots): `player_history` 20/20 (was 0/20), `player_stat` 27/30 (was 6/30; the three left are a log hand-off, a dated read the compiler declines - ISSUES, "The compiler cannot unset a slot's season" - and F055, where the template answers a different question). `threshold_count`'s notes are one function (`_threshold_count_notes`), called by the template and by `compose.present`, which restated two of its sentences. Golden over the 308 recorded questions: 308/308 identical.
- The compiler declines a team's total of its players' triple-doubles or double-doubles instead of refusing it as a ranking with no measure, so the refusal names the real cause (`refusals._team_boolean_count`).
- A game log against one team that asks for both season types ("including playoffs") and names no year is a career of meetings, the same reading "last N vs <team>" already gets; and a team's count of its players' triple-doubles or double-doubles, arriving as a `leaderboard` with the team filed, is refused for that cause (`refusals._team_boolean_count`) rather than "no ranking reads triple_double".
- A team's game log states the point differential under the model's other spellings of the stat (`points_differential`, `point_differential`, `differential` - "KNICKS point differential over the last 7 games" listed the right seven games with no total line on day6).
- **Four more of the shorter prompt's drifts met by text rules** (ISSUES #223, the day5 batch): a `with_without` naming no teammate and asking for "the last N games" is the team's log (`game_log` reads both season types for "last N"; "KNICKS point differential over the last 7 games" had answered the regular season's last seven); a game of each series drops a filler `order`/`limit` ("Ayton stats in game 4 playoff games" showed one of the two); two players "vs" over a span of seasons with no compare word is the pair's meetings (`player_matchup`, which honors `since`; "jokic vs cade since 2022" fell through as a comparison); and `team_leaderboard` shows a tie at its cut whole ("nba team with least playoff wins since 2022" with the model's limit of 1 listed the Nets alone where the Hornets and Wizards share the zero).
- A subject the router filed as the TEAM under a team-only parent leaves the team slot once the words assign a child ("how many playoff games has embiid won?" arrived as an outlook for a "team" named Joel Embiid, and the stray slot refused the compiler's read); `single_game_high` reads a threshold off the text too ("highest fg percentage in a 30 point triple double game ever" lost its 30-point line when the model stopped filing one).
- **The reading takes the router's no-names and inventions out of the slots** (the second day5 run). A router player that is no name - a rank word or a phrase of the question ("most", "most 30+ point games"), or a name the question holds only by a team's word ("magic vs nets" is not Magic Johnson) - is `Subject.filler`, dropped from `player`/`players` rather than left for a template to resolve into "No player found matching 'most'"; a team filed as the opponent on a question that sets its subject against nobody is dropped ("PHI record when Embiid and Paul George play" arrived with the Pacers); a non-team in `team` that is the opponent's own word goes ("Centers stats game log vs kings" arrived with the Los Angeles Kings); a team read as a player is the log's own subject beside its opponent; a position group on a `leaderboard` is handed to the compiler as the player (the template raises on one; "by a shooting guard" had ranked the whole league); `undo_name_completion` cuts back a completion that resolves to nobody ("Derozan Valenčić"); a `record_when` games-won question reads `wins`/`losses`; a count with no stat takes the one line's column; and `_names_a_count` sets a "30+ points" pair aside, so the model's `limit: 1` on "who had the most 30+ point games" is filler. Golden 308/308, `check_routing` 140/140.
- **What the shorter router prompt moved on the first live run, fixed in code** (day5 of the yardstick on the 2c-ii tree; none by a prompt edit). The reading no longer takes a router "name" holding a digit, a plus sign, a leading rank word or the word "player" as a player ("most 30+ point games", "most", "Most Player in 15th Season Played" - `subject._NO_NAME_HAS`); the word "fingerprint" names the intent wherever the model filed it (`_FINGERPRINT_NAMED`: "compare fingerprints for embiid vs jokic" arrived as `player_compare`); "record when X and Y play" with no threshold is `with_without` under any of the team intents (`_PLAYED_TOGETHER_REROUTABLE`: it arrived as `head_to_head` with an invented opponent); "how many games did X play" under a game log is the line's count, not a log of his last game; a sided NetPoints ranking keeps its side ("top 10 in adjusted offensive netpoints" arrived as the total); a filler `limit` on a count or a single-game high (the leaderboard parent's "who" = 1) is dropped so the runner-ups return, and a filler limit on a named game log is dropped whatever its number (limit 15 from "less than 15 fga" listed fifteen games across two season types); `player_splits` joins `SUBJECT_RESTORABLE_INTENTS` and is assigned under `head_to_head`/`team_record` too ("show me Embiid's splits against boston" arrived as the two teams meeting, Embiid dropped), and the games-won reading of `record_when` under `team_outlook` ("how many playoff games has embiid won?" arrived as an outlook for a "team" named Joel Embiid). `check_routing` 140/140, golden 308/308 unchanged.
- **The seven child intents leave the router's prompt and schema** (ROADMAP plan item 2, step 2c-ii - the first `ROUTER_PROMPT` shrink). `threshold_count`, `single_game_high`, `player_history`, `shot_distance`, `player_splits`, `record_when` and `streak` are gone from `ROUTER_PROMPT`'s intent list and worked examples and from `ROUTER_SCHEMA`'s enum (23 intents to 16); the subject reading assigns each from the question's words under the parent the router now chooses (`subject.KIND_ASSIGNED_INTENTS`, the step before this one), and `scripts/check_routing.py`'s cases for them assert the child as before, reached through the reading. The reachability tests count the kind-assigned set beside `CODE_ASSIGNED_INTENTS`, and the grammar-exclusion test covers both.
- **The compiler says an intent's own default point the way that intent's template does** (ROADMAP plan item 2, step 2a - presentation parity). The new `compose.present` answers `game_log`, `player_stat` (a narrowed average), `threshold_count`, `single_game_high` and `record_when` at their own point with the template's own text and `data` - the headline, notes and typed keys the page renders - over the compiler's settled player, span and narrowing: `game_log`/`player_stat`/`record_when` through their templates' readers that take a settled narrowing (`_player_game_log`, `_player_game_log_mixed`, `_box_score_player_stat`, `_record_when_query`/`_record_when_answer`), and `single_game_high`/`threshold_count` over the compiler's own rows and their phrasing helpers. It applies only where `check_scope` passes and no word of the question moved the point; everything else keeps the compiler's own sentence. Measured over the recorded corpus (`~/association-research/intent-shrink/parity.py`, text and `data` both identical to the template on the same slots): `threshold_count` 18/18 (was 1), `single_game_high` 10/10 (was 1), `record_when` 10/10 (was 0), `game_log` 19/26 (was 2; all 19 player logs, none of the 7 team logs), `player_stat` 6/29 (was 0; the 6 narrowed reads - the other 23 read the season line); `player_history` (season lines) and `player_splits` stay 0. Fixed on the way, in the compiler: a per-game usage rate printed as "2435.5%" (#222 - `usage_pct` is stored as a percent, not a fraction); a composed answer carried the partial-season caveat twice (the agent appends it; `compose.answer` and `run_team` appended it too, and `TeamResult.coverage_note` is gone); "past 4 seasons" of a player's history listed his FIRST four (ordered by the label ascending, now newest first and headed "most recent N seasons"); a `single_game_high` question without "in a game" ranked per-game averages ("what was the highest scoring game this year" named Luka Doncic's 33.5 average, not Bam Adebayo's 83); "most games with 15+ assists" declined for want of a line (the router's own stat at its threshold is the line); "who had the most 30+ point 10+ rebound games" listed games instead of counting them by player; a log or top-games read never showed a rebuilt game (`minutes` is exempt from the rebuilt-line rule, as it is for `game_log`); `own_team` ("lebron stats as a starter for Miami") was ignored, averaging 1,612 career starts where the template counts his 294 for Miami; `season_type_unstated` ("including the playoffs") is honored for a named player instead of declining; "how many times has embiid fouled out" reads as `fouls >= 6`, the router's own line; and a player the reading placed beside a team subject ("stats for sixers when maxey scored 20+") is no longer moved into `team` and resolved as a team called "Maxey". Golden over the 308 recorded questions: 307 identical, the one move being `player_history`'s newest-first order.
- **Seven child intents are assigned from the question's own words, gated on the subject's kind** (ROADMAP plan item 2, step 2c-i - the mechanism that lets them leave the router's prompt in 2c-ii). `subject.KIND_ASSIGNED_INTENTS` (`threshold_count`, `single_game_high`, `player_history`, `shot_distance`, `record_when`, `streak`, `player_splits`): when the router chose a parent (`game_log`, `player_stat`, `leaderboard`, `other`; `team_record` and its siblings for a streak or a record), the words that tell the child from the parent ("30+ point games", "in a single game", "over the past 4 seasons", "how far", "record when ... 20+", "streak", "splits") assign it - only for a subject kind the child can be about, which is what keeps "how many times did the 76ers play boston" (two teams) and "who lead the league in avg 3 point distance" (no player) where they are: measured over 352 recorded (question, intent) pairs, no question of another intent moves. The child's slots come from the router's own stages run again under it - the new `router.settle(intent, slots, question)`, which `route()` now shares - so the threshold, the seasons count, a streak's kind and a split are read the one way the router reads them, and a parent's derived slot (a game log's `since` for "the past 4 seasons") does not survive into a template that refuses it; the reading's player is restored where the child needs one. `apply_subject` returns the settled intent (the stages may decline: a count with no threshold in the text is a ranking and stays the router's question), and the agent's fast path applies the reading before the no-template check, so the router's `other` is a parent too. With the children still in the router's enum this is a no-op on every recorded route: golden over the 308 recorded questions unchanged. Beside it: `router._THRESHOLD` is built from the one spelling list (`_THRESHOLD_SPELLINGS`, now with "pt" - "30 pt games" read no threshold and became a ranking), a ceiling-only count keeps its intent and the template reads the line as the count ("Sga games with under 14 fta", with `freeThrowsAttempted` from the question rather than the model's nearest made-stat), `shot_value` is read off the text for a distance or a chart (`_route_shot_value`, the value the shot_distance worked example used to teach), and `shot_distance`/`player_history` join `PLAYER_REQUIRED_INTENTS`. `Subject` carries the `question` it was read from.
- **The compiler and the refusals read the subject reading instead of re-deriving it** (ROADMAP plan item 1, step 3e - the reading is now the one place the subject is decided). `compose.move.move_point`, `repair`, `team_move_point` and `compose.answer` take the `Subject` the agent read (and read one themselves for a caller without it): the position group, the filler word and the team in `player` (`_drop_position_only_player`, `_drop_filler_or_team_player`), the dropped subject and the teammates filed as the opponent (`repair`), the team the router left out (`team_move_point`) and the position `_everyone_point` filters by all come off it. `refusals.unanswerable` takes it too, and the team-in-`player` refusal reads the kind. The reading itself no longer takes a position phrase ("shooting guard", F056) or the router's filler word ("player", F099) as a player - `FILLER_PLAYER_WORDS` and `POSITIONS` now live in `templates.common` (`compose.move` re-exports `POSITIONS`), and a router `team` that resolves to no team (`"any_team"`) is not the subject's team. Golden over the 308 recorded questions unchanged.
- **`entities.scope_from_question`, `entities.player_record_against_a_team` and `refusals.pair_from_opponent` are gone: the subject reading settles the slots and the intent.** `subject.apply_subject` takes the last two repairs (a player's name filed in `team` - "Podziemski game log without curry", "Will Riley last 5 game s" - and a team in `team` that displaced the player the question names, or the sides of a team-vs-team log filed backwards) and returns `Applied(decisions, dropped, intent)`, where `intent` is what the subject's shape settles (`Subject.intent`): a player's record against a team is `with_without`, not two franchises meeting; a player filed as the `opponent` on a pairable intent is the pair relation (`player_matchup`); and two new readings from the chain's own measured bugs - a pair the router sent to `with_without` is `player_matchup` ("steph curry record vs lebron regular season without kd", yardstick-v2 F114, which answered a team's record with and without Durant), and a pair the question *compares* sent to `player_stat` with the second player as the `opponent` is `player_compare` ("compare Jaylen Brown and Jason Tatum's netpoints ...", which fell through on "no team matching 'Jason Tatum'"). A pair means exactly two players. `Subject.opponent` also reads the router's `team` for a "vs" word nothing resolves ("karl towns stats vs netslast 5 games"). The agent's fast path is now: nicknames, the reading, a fingerprint's dropped players, the reading applied, name-completion undone, the team-only refusal. `scripts/check_routing.py` takes the reading's intent.
- **The subject reading writes the restored player, the player's own team and the team subject.** `subject.apply_subject` puts back the one player the question names where the router left the player out (`PLAYER_REQUIRED_INTENTS`, `SUBJECT_RESTORABLE_INTENTS` - subject or companion: "celtics record with 20+ points from jayson tatum" reads as the Celtics with Tatum as the condition, and `record_when`'s player slot is the condition player), a player's own team from "for <team>" beside him into `own_team` with the tenure-to-career default (`OWN_TEAM_RESTORABLE_INTENTS`; `Subject.named_season` is the season the question itself names, never the router's default; `Subject.own_team` needs the player named before the phrase - F087), and a team the question names as its own subject into `team` (`TEAM_SUBJECT_RESTORABLE_INTENTS`, with `leaderboard`'s `team_restored` marker). `scope_from_question` loses `needs_player`, `restore_subject`, `restore_team`, `restore_team_subject` and `intent` with the steps they gated (`_scope_from_question_restore_player`, `_own_team`, `_team_subject` and their two wrappers, deleted); what remains of it is the router filing a player's name in `team`, and a team in `team` that displaced the player. Golden over the 308 recorded questions unchanged.
- **The subject reading writes the opponent team.** `subject.apply_subject` now takes `con` and `intent` and puts the team the subject is set against where a template reads it - into `opponent`, and out of `players` ("how did curry do against the celtics" put the Celtics there, where "boston" is Brandon Boston Jr.), `team` beside a player a template reads ("compare curry and lebron vs the celtics") and `teams` ("Keyonte George against blazers") - and drops an `opponent` naming a player the slots already ask about; each a recorded decision. `Subject.opponent` is the question's own "vs X" whenever there is one, else the router's `opponent` where it names a team the question holds. Those were `scope_from_question`'s steps (`_scope_from_question_opponent`, `_unheld_opponent`, `_drop_opposing_team`, `_team_in_players`, `_opponent_is_a_subject`), deleted; its team-subject restore reads the question's "vs" itself now instead of an `opponent` slot an earlier step filled. Golden over the 308 recorded questions unchanged.
...
- **Fixed:** two players' games on the same date - a position group's log ("Centers stats game log vs kings") or a league-wide read - came out in DuckDB's parallel scan order, which is not stable run to run: the relation's `rows_sql` (`player_games.py`) ordered by date alone. `ROWS_TIEBREAK` (event, then player name) is now appended to that one ORDER BY, so every reader gets a deterministic order - measured against the real warehouse, `scripts/preview_answers.py` over `live_day2.jsonl` now returns byte-identical text across five runs where it previously reordered Donovan Clingan and Robert Williams III.
- **Fixed:** a career answer composed by `query/compose` (not a template) named its span by the relation's box-score floor - "in the regular season career (1994 on)" - even for a named player, which is true of every player and says nothing about the one asked about. `compose.core._player_own_seasons` reuses `templates.players._seasons_on_record` (the same read `threshold_count`/`single_game_high` already make) so a plain career now names the player's own first and last season instead, e.g. "(2019-2026)" for Shai Gilgeous-Alexander - measured on the real warehouse (`live_day3.jsonl`, 277 questions): the 479-game count is unchanged, only the span wording.

## 4.4.0 - 2026-09-25
- The subject reading's list of ordinary words ships with the package (`query/words.txt.gz`, from Debian's `wamerican` 2020.12.07, with its SCOWL notice in `words.COPYRIGHT`) instead of being read from `/usr/share/dict/words`. On a machine without that file, "Best record from 2010-11 to 2018-19" was read as a question about Travis Best: the fallback kept only the month abbreviations, so any word that is also a whole player name ("best", "head", "pointer") became a player. GitHub's runner is such a machine, which is why CI failed from `7b8b7a2` on while the same tests passed locally. The shipped list holds exactly the 64,005 words the system file gave, so answers on a machine that has the file do not change. A wheel missing it now raises instead of silently answering differently.
- The subject reading writes the first field: `subject.apply_subject` puts the players the question was read to be about into `player`/`players` - a router name the question never held replaced by the question's own spare name, or refused by name - before `override_invented_players` runs over the same field (which the golden then shows finding nothing to do). Each replacement is a recorded decision. A router name the question spells differently - a bare "Jokic" left bare, "Jaylen Tatum" for the question's "tatum", "Deron Williams" two edits from "derozan" - takes the question's own resolved name (`Subject.named`), which was `_question_derived_player`'s job.
- Decisions as data: `query/decisions.py`'s `Decision(stage, field, before, after, reason)` records what was decided on the way to an answer - a reading of the question, an override of a routed field, a default - kept on the history record (a `decisions:` section of JSON lines beside the trace), on `Answer.decisions`, on the API's `AnswerResponse.decisions`, and shown by the page under a "decisions" disclosure; never parsed back out of the trace. The subject reading is the first producer: every fast-path answer now records who the question was read to be about, beside the repair chain that still writes the slots.
...
- The web interface can annotate an answer: a note control under each answer posts to the new `POST /api/notes`, which appends a timestamped `[note ...]` line to that answer's own `.history/` file, guarded by name the same way `GET /api/artifacts/{name}` is. `association.query.history.append_note` writes the line (escaping an embedded newline so the file stays one thing per line), and the answer event now names its own history file (`AnswerResponse.history_file`, `association.web.runner.Answered`) so the page has something to attach a note to. More than one note per answer is allowed; each save appends rather than replacing the last.
- The compiler reads an ordinal season over everyone ("Most points in 15th season played": each player's own 15th regular season, on the league read), a team's name in the `player` slot as the team narrowing of a league-wide read ("oklahoma city thunder all-time triple doubles": 180, by player, the whole count behind a windowed list), and the router's filler `player` word as no player.
