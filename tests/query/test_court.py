...
def test_render_court_html_includes_title_and_subtitle():
    html = render_court_html("Test Player", "season 2024 - 1/2 (50.0%) shown", [])
    assert "Test Player" in html
    assert "season 2024" in html


def test_render_court_html_marks_makes_and_misses_distinctly():
    shots = [
...
def test_render_court_html_is_theme_aware():
    html = render_court_html("Test Player", "sub", [])
    assert "prefers-color-scheme: dark" in html


def test_render_court_html_valid_svg_wrapper():
    html = render_court_html("Test Player", "sub", [])
