...
def test_names_the_losing_side_from_the_same_game(con: duckdb.DuckDBPyConnection) -> None:
    """The same row, read from the other team's player: an opponent, a venue
    and a result relative to THAT player's own team, not always the home
    team's. Portland were away, so their side of it reads "@ GS" - saying
    "vs" for both sides was wrong on exactly half of all games, and placing
    the game is the whole purpose of the label."""
    assert game_label(con, "2", "100") == "2025-04-12 @ GS, L 104-118"
