...
def test_write_rows_then_exists(tmp_path):
...
def test_write_rows_is_atomic_no_leftover_tmp_file(tmp_path):
    path = tmp_path / "data.parquet"
    storage.write_rows(path, [{"a": 1}])
    assert not path.with_name(path.name + ".tmp").exists()


def test_write_rows_empty_list_is_noop(tmp_path):
    path = tmp_path / "data.parquet"
    storage.write_rows(path, [])
    assert not path.exists()


def test_write_row_single_dict(tmp_path):
    path = tmp_path / "row.parquet"
    storage.write_row(path, {"x": 1, "y": "hello"})
    assert storage.exists(path)


def test_exists_false_for_zero_byte_file(tmp_path):
...
def test_mark_complete_then_is_complete(tmp_path):
...
def test_mark_complete_creates_parent_dirs(tmp_path):
